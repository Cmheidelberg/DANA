...
public class WorkflowJson {
	...
	/**
	 * Use the json to populate the workflows array with nodes from the wings
	 * workflow. This populates with any dataset, parameter and step notes, as well
	 * as creating a relationship of input and output nodes for each node.
	 */
		...
		for (int i = 1; i < key.length; i++) {
			try {
				...
				if (tmp.getValueType().toString().equalsIgnoreCase("object")) {
					...
					System.out.println(
							"Cannot get value from this method. It only returns JsonObjects. "
